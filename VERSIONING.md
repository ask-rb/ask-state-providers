# Versioning — ask-state-providers
...
ask-state-providers is pre-1.0 (`0.y.z`). Until 1.0.0:

- **PATCH** (`0.4.8` → `0.4.9`) — bug fixes and other
  backwards-compatible corrections. No new features, no public API changes.
- **MINOR** (`0.4.8` → `0.5.0`) — new features **and**
...
Patch numbers increase by exactly one per release. From `0.4.8` the next
patch is `0.4.9` — never `0.4.11`, never a skip, never a jump. Patch
...
Every `ask-*` gem — ask-state-providers included — and `yamine` is released
...
gemchain guard ask-state-providers           # blast radius: who depends on us
gemchain update ask-state-providers 0.4.9 --dry-run
gemchain update ask-state-providers 0.4.9 --test-only
gemchain update ask-state-providers 0.4.9
```
...
2. **Version** — bump `lib/ask/state/providers/version.rb` by exactly one
...
5. **Build** — `gem build ask-state-providers.gemspec` succeeds and the
...
- `lib/ask/state/providers/version.rb` at `HEAD`
- the `vX.Y.Z` git tag
...
- Fix a bug with no API change → `0.4.9` (patch).
- Add a new state provider → `0.5.0` (minor).
- Remove or rename a public method → `0.5.0` (minor — breaking is allowed
  pre-1.0, but only ever in a minor).
- From `0.4.8`, **never** jump to `0.4.11` for a fix, and **never**
  release `0.6.0` for a one-line bugfix.
- Two fixes in a row → `0.4.9`, then `0.4.10` — one step each time.
- Dependency example: releasing `ask-core` runs
  ...
  ask-state-providers by one patch, commits, publishes, tags, and pushes —
